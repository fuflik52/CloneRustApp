...
export default function Settings() {
  ...
  const actionMatches = (a: CustomAction, b: Omit<CustomAction, 'id'>) => {
    const aName = a.name.trim().toLowerCase()
    const bName = b.name.trim().toLowerCase()
    const aCmds = a.commands.map(c => c.trim()).filter(Boolean).join('\n')
    const bCmds = b.commands.map(c => c.trim()).filter(Boolean).join('\n')
    return aName === bName && aCmds === bCmds
  }
  ...
  const saveAction = async (action: Omit<CustomAction, 'id'>) => {
    if (actions.some(a => actionMatches(a, action))) {
      showToast('Такое действие уже существует', 'info')
      return
    }
  ...
  const updateAction = async (updated: CustomAction) => {
    setActions(prev => prev.map(a => a.id === updated.id ? updated : a))
    setEditingAction(null)
    try {
      await fetch(`/api/servers/${serverId}/actions/${updated.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updated)
      })
  ...
  return (
    <div className="settings-page">
                    ...
                    <div className="action-controls">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setActionMenu({ id: action.id, x: rect.left, y: rect.bottom + 6 })
                        }}
                      >
                        <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            {actionMenu && (
              ...
              >
                <button
                  ...
                  onClick={() => {
                    const a = actions.find(x => x.id === actionMenu.id)
                    if (a) setEditingAction(a)
                    setActionMenu(null)
                  }}
                >
                  Редактировать
      ...
      {showPresetModal && (
        <PresetActionsModal onClose={() => setShowPresetModal(false)} onSelect={saveAction} existingActions={actions} />
      )}
...
function PresetActionsModal({ onClose, onSelect, existingActions }: PresetActionsModalProps) {
  ...
  const presetExists = (preset: typeof presetActions[0]) => {
    const presetName = preset.name.trim().toLowerCase()
    const presetCmd = preset.command.trim()
    return existingActions.some(a =>
      a.name.trim().toLowerCase() === presetName &&
      a.commands.map(c => c.trim()).filter(Boolean).join('\n') === presetCmd
    )
  }
      ...
      <div className="preset-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="preset-list">
          {presetActions.map((preset, i) => {
            ...
            <div
              key={i}
              className={`preset-item ${exists ? 'disabled' : ''}`}
              onClick={() => { if (!exists) handleSelect(preset) }}
            >
...
function EditActionModal({ onClose, action, onSave }: EditActionModalProps) {
  const [step, setStep] = useState(1)
  const [name, setName] = useState(action.name)
  const [group, setGroup] = useState(action.group)
  const [accessLevel, setAccessLevel] = useState<CustomAction['accessLevel']>(action.accessLevel)
  const [allowOffline, setAllowOffline] = useState(action.allowOffline)
  const [selectServer, setSelectServer] = useState(action.selectServer)
  const [confirmBefore, setConfirmBefore] = useState(action.confirmBefore)
  const [commands, setCommands] = useState<string[]>(action.commands.length ? action.commands : [''])
  const [showAccessDropdown, setShowAccessDropdown] = useState(false)
    ...
    <div className="settings-modal-overlay" onClick={onClose}>
      <div className="action-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-card">
          <div className="modal-card-header">
            <div className="header-text">
              <p className="header-title">Редактировать действие</p>
              <p className="header-subtitle">{step === 1 ? 'общие настройки' : 'команды'}</p>
            </div>
            <div className="header-icon">
              <button className="close-btn" onClick={onClose}>
